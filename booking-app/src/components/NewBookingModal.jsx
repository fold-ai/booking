  ...
  const save = async () => {
    ...
    try {
      ...
      if (!clientId && form.newClientName) {
        const c = await addClient({ name: form.newClientName, phone: form.newClientPhone, address: form.newClientAddress, email: '', tags: [], notes: '' })
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button onClick={save} className="btn-accent" disabled={saving || !form.serviceId || (!form.clientId && !form.newClientName)}>
