...
export default function BusinessProfile() {
  ...
  return (
    <Shell>
      <div className="relative h-64 overflow-hidden rounded-3xl bg-ink-100 sm:h-80">
        {business.heroImageUrl ? (
          <img src={business.heroImageUrl} alt="" className="h-full w-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none' }} />
        ) : (
          <div className="grid h-full w-full place-items-center text-7xl">{type.emoji}</div>
        )}
      ...
      <header className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-end gap-4">
          <div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-2xl border-4 border-white bg-ink-100 text-3xl shadow">
          ...
          <div>
            ...
            <h1 className="font-display text-4xl text-ink-800 sm:text-5xl">{business.name}</h1>
            <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-ink-500">
              ...
              {business.email && <span className="inline-flex items-center gap-1"><Mail size={13} /> {business.email}</span>}
            </div>
          </div>
        </div>
        {business.acceptingBookings && (
          <Link to={`/book/${business.slug}`} className="btn-accent">
      ...
      {business.tagline && (
        <div className="mt-6 font-display text-2xl italic text-ink-600">"{business.tagline}"</div>
      )}
