...
export function BusinessProvider({ children }) {
  ...
  const createBusinessForUser = async ({ name, type, slug, city, phone, schedule, slotMinutes }) => {
    ...
    //    Наприклад коли Signup wizard викликав це двічі, або юзер перезавантажив сторінку.
    {
    ...
    if (be) {
      // Якщо це duplicate constraint на slug — спробувати ще раз з іншим slug
      if (be.code === '23505' || /duplicate/i.test(be.message || '')) {
        ...
        // Повертаємо результат retry
        await seedDefaults(retry.data.id, typeDef)
        await refresh()
        return fromBusiness(retry.data)
      }
      throw be
    }
