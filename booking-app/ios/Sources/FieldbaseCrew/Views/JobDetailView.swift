...
struct JobDetailView: View {
  ...
  var body: some View {
    let client = state.clientsById[booking.clientId]
    return ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        VStack(alignment: .leading, spacing: 6) {
          ...
          Text(booking.serviceName ?? "Service").foregroundStyle(Theme.ink400)
        }
  ...
  private var infoCard: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      if let phone = state.clientsById[booking.clientId]?.phone {
    ...
    VStack(alignment: .leading, spacing: 8) {
      ...
      HStack(spacing: 8) {
        ForEach(BookingStatus.allCases, id: \.self) { s in
          Button {
            Task { await state.setStatus(booking, status: s, notes: notes) }
          } label: {
            Text(s.label)
  ...
  private var notesCard: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      Button {
        Task { await state.setStatus(booking, status: booking.status, notes: notes) }
      } label: {
