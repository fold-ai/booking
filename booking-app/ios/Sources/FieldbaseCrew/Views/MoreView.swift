        ...
        VStack(alignment: .leading, spacing: 20) {
          ...
          if let business = state.role?.business {
            VStack(alignment: .leading, spacing: 8) {
              ...
              Text("All features above are already available at fieldbase.app. Open your booking page or admin dashboard from any browser.")
                .font(.caption).foregroundStyle(Theme.ink400)
              Link(destination: URL(string: "https://fieldbase.app/book/\(business.slug)")!) {
                HStack(spacing: 4) {
