...
struct Booking: Identifiable, Codable, Hashable {
  ...
  // Витягуємо назву сервіса джойном — окремо, не з рядка
  var serviceName: String? { _serviceName }
  private var _serviceName: String? = nil

  // Для views що показують щось коли address nullable у БД
  // (схема має `text` nullable, тому декодуємо порожнім рядком)
